...
class DepartementAttendance extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $employeesQuery = Pegawai::where('unit_kerja_id', $departmentId)
            ->where('id', '!=', $manager->id);
        ...
        return view('livewire.attendance.departement-attendance', [
    ...
    public function showAttendanceDetail($id)
    {
        ...
        // Check if this is an absent record (id starts with "absent_")
            ...
            $employee = Pegawai::with(['jabatan', 'unitKerja'])
                ->where('id', $employeeId)
                ->where('unit_kerja_id', $departmentId)
                ->first();
                
            if (!$employee) {
                $this->dispatch('showAlert', [
                    'type' => 'error',
                    'message' => 'Data pegawai tidak ditemukan atau Anda tidak memiliki akses.'
                ]);
                return;
            }
